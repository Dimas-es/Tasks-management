...
export const ChallengeProvider: React.FC<ChallengeProviderProps> = ({ children }) => {
  ...
  const getChallenges = () => {
    try {
      ...
      if (Array.isArray(parsedChallenges)) {
        ...
        console.error("Format tantangan yang ada tidak valid:", parsedChallenges);
        return [];
      }
    } catch (error) {
      console.error("Terjadi kesalahan saat memparsing tantangan dari local storage:", error);
      return [];
    }
  };
  ...
  const addChallenge = (newChallenge: {
    ...
    if (Array.isArray(challenges)) {
      ...
      console.error("Tantangan yang ada bukan dalam format array:", challenges);
...
export const useChallenge = () => {
  ...
  if (!context) {
    throw new Error("useChallenge harus digunakan di dalam ChallengeProvider");
  }
  return context;
};
